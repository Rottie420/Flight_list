...
def canonicalize_name(name):  # type: (str) -> str
  if not isinstance(name, (str, bytes)):
      # If name is not a string or bytes, convert it to a string
      name = str(name)
...
def get_client_cert(config,
                    repository_name):  # type: (Config, str) -> Optional[Path]
...
def get_package_version_display_string(
    package, root=None):  # type: (Package, Optional[Path]) -> str
